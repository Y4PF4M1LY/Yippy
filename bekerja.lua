-- Auto Fishing Script (0.5s Cast + 0.5s Fill | Delta & Solara Compatible)
-- Changes:
--   • UI centered on screen
--   • VirtualInputManager replaced with mouse1press/mouse1release/mouse1click (Delta/Solara)
--   • Click loop uses task.spawn instead of Heartbeat connection
--   • instantCatch defaults to TRUE
--   • Solara: falls back gracefully if mouse globals are slightly different
...
local config = {
    enabled      = false,
    instantCatch = true,       -- Default ON
    autoRecast   = true,
    castHoldTime = 0.5,        -- 0.5s cast hold
    savedPosition = nil,
    clickDelay   = 0.009,      -- ~55 clicks/sec -> fills bar in ~0.5s
}
...
-- ─────────────────────────────────────────────────────────────
--  Executor-safe input layer
--  Delta & Solara both expose mouse1press / mouse1release / mouse1click as globals.
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
--  Auto-click loop (task.spawn — no Heartbeat connection issues)
-- ─────────────────────────────────────────────────────────────
...
local function buildUI()
    ...
    frame.Position         = UDim2.new(0.5, -W/2, 0.5, -H/2)  -- centered
    ...
    -- Patch bottom of title bar so corners are square there
    ...
    titleLbl.Text                  = "🎣 Ultimate Fishing System"
    ...
    local toggleBtn  = makeBtn("⏹ DISABLED",          Color3.fromRGB(210,45,45),  0.9, 38, 0.05, 10)
    ...
    badgeLbl.Text           = "⚙ Delta & Solara Compatible"
